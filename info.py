...
AUTH_CHANNEL = [int(fch) if id_pattern.search(fch) else fch for fch in environ.get('AUTH_CHANNEL', '-1002563875182').split()]
MULTI_FSUB = [int(channel_id) for channel_id in environ.get('MULTI_FSUB', '-1002563875182 -1002332123168').split() if re.match(r'^-?\d+$', channel_id)]  # Channel for force sub (make sure bot is admin)
